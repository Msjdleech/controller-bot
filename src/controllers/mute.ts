...
const mute = async (ctx: Context) => {
  ...
  const message =
    `User <a href="tg://user?id=${id}">${firstName} ${lastName}</a> has been restricted, until 3 hour.`;
  const result = await ctx.restrictChatMember(
    id,
    { can_send_messages: false },
    { until_date: Math.floor(Date.now() / 1000) + (3600 * 3) },
  );
